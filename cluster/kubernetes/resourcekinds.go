...
func makeStatefulSetPodController(statefulSet *apiapps.StatefulSet) podController {
	...
	rollout := cluster.RolloutStatus{
		...
		// There is no Avaliable parameter for statefulset, so use Ready instead
	...
	// rolling update
	if statefulSet.Spec.UpdateStrategy.Type == apiapps.RollingUpdateStatefulSetStrategyType && statefulSet.Spec.UpdateStrategy.RollingUpdate != nil {
		if statefulSet.Spec.Replicas != nil && statefulSet.Spec.UpdateStrategy.RollingUpdate.Partition != nil {
			// Desired for this partition
			rollout.Desired = *statefulSet.Spec.Replicas - *statefulSet.Spec.UpdateStrategy.RollingUpdate.Partition
		}
	}
	...
	if statefulSetStatus.ObservedGeneration >= objectMeta.Generation {
		// the definition has been updated; now let's see about the replicas
		status = StatusUpdating
		if rollout.Updated == rollout.Desired && rollout.Ready >= rollout.Desired && rollout.Outdated == 0 {
			status = StatusReady
		}
	}
